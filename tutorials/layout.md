#layout tutorial
welcome to the layout tutorial!
this tutorial explains how to style your slides with layout-positioning

---
## how slidenotes interprets your data - the block way

slidenotes reads your data and creates blocks out of it. then it tries to find the best solution how to position this blocks on the slide. 
this works well for most use cases and therefore makes writing slidenotes really fast. 
but sometimes you want more control. 

we have thought about how we could implement an easy and intuitive way to achieve this solely inside the texteditor in a fast way and came up with a toolset which we hope you will love

---
## introducing `+++layout` blocks 

the first tool to achieve easy and fast positioning is the layout-block. 
it serves as a container for all elements inside, so you surround your content with it. 
you can select yet written content and then containerised it inside a layout block with the help of the toolbar to achieve faster result then writing by hand. 

now you have a container around the elements. although it does not have any positioning effect yet you can give it a background-image by adding an image-tag `![](image)` in the first inner line of the layout

+++code
 +++layout
 ![](backgroundimage)

 ###some text over it
 +++
+++

+++layout
![](backgroundimage)
###some text over it
+++
---
## positioning of the layout block `:left`

the layout block itself can have three states for now. the first state is the `+++layout:left` state. to achieve this you add `:left` to the header of the layout block. 

whenever a layout-left block appears the rest of the slide will be divided into two parts. all content inside the layout-left block will appear on the left side, all content **after** the layout-left block will appear on the right side of the block:

+++layout:left
![](backgroundimage)

# my left side
+++

+++code
 +++layout:left
![](backgroundimage)

# my left side
 +++

+++

---
## positioning of the layout block `:right`

the second state is the `+++layout:right` state. to achieve this you add `:right` to the header of the layout block. 

whenever a layout-left block appears the rest of the slide will be divided into two parts. all content inside the layout-left block will appear on the right side, all content **after** the layout-left block will appear on the left side of the block:

+++layout:right
![](backgroundimage)

# my right side
+++

+++code
 +++layout:right
![](backgroundimage)

# my right side
 +++

+++

---
## positioning of the layout block `:left` and `:right` combined

as you can see the left and right have different types how to divide the slide. a soley `:left` occupies 30% of the slade width, while a soley `:right` occupies 50%.
you can also combine left and right to achieve effects like dividing the slide into three areas:

+++layout:left
![](backgroundimage)

# my left side
+++
+++layout:left
![](backgroundimage)

# my 2nd left side
+++
+++layout:right
![](backgroundimage)

# my right side
+++

---
## positioning of the elements inside the layout `:inline`

the third state is the `+++layout:inline` state. it takes the whole available width, but its height is determined by the elements inside. instead of positioning the layout it influences the positioning of the elements inside the layout by displaying them (if possible) in one line each next to each other. elements after the `:inline` block continue normaly:
 
+++layout:inline
![](backgroundimage)

###first block

###second block

###third block
+++
+++code
 +++layout:inline
 ![](backgroundimage)

###first block

###second block

###third block
 +++
+++

---
## sticky titles and imageblocks

to make it even easier we introduced the creating of blocks for sticky titles and images with an image subtitle:

+++layout:inline

###sticky title
the sticky title sticks to this textblock as there is no empty line between them

###imagetitle
![](lapa)
this is a friendly dog

###listtitle
1. a small
2. list 
3. with a
4. title
+++

*press escape or the x down left to see sourcecode of this slide*
---
## the `+++hidden` block 

at last we want to introduce you to the `+++hidden` block. 
with the hidden block we want to solve the following problem:

lets say you want two slides, which looks nearly the same, but in the second slide you would like to pop up something new. 
the intuitive choice would be - and that is a technique you would use in a pixel- and mouse-oriented program like LibreOffice Impress - to just copy the first slide and then alter the second slide by adding something. 

+++code:options
language:markdown
---
## my dog

![](lapa)
---
## my dog

![](lapa)

isnt she cute?
+++


if the order is still the same this *could* work in slidenotes. but if you break the order or just that the second slide holds more elements the result could not be what you expected it to be. the image first is more down and/or bigger, then flips up to make room for the text for example.
this is because slidenotes tries to get the best out of each slide and does not compare it with the other slides - it interprets it slide by slide. as it does not know that the position of the elements should be exactly the same it orders them by what it gets from each slide anew. 

you *could* help yourself with constructing some good defined layouts but that would be complicated and surely not very fast. so thats what the `+++hidden` block is for.
---
## the `+++hidden` block in action
 
this is one slide with a hidden block. it is copied and shows the hidden part only on the copy (or second slide) 

+++layout:inline

someone is *hiding* between this textblock and the codeblock to the right... guess who?


+++hidden
##lapa
![](lapa)
isnt she cute?
+++

+++code:options
language=md
---
 +++hidden
##lapa
![](lapa)
isnt she cute?
 +++
+++
+++

*you can exit this slide on both versions of it and edit the slide directly*
---
## more then one `:hidden` object on one slide

you can hide more then one object on a slide. for each hidden element there will be one more copy of the slide generated and they will appear in the same order as they were defined in the slide:

+++inline
+++hidden
### first element
+++

+++hidden
### second element
+++
+++hidden
### third element
+++
+++hidden
### fourth element
+++
+++
+++code:options
language:md
---
 +++inline

 +++hidden
### first element
 +++

 +++hidden
### second element
 +++

 +++hidden
### third element
 +++

 +++hidden
### fourth element
 +++

 +++
+++



---
## thats it. for now

with the tools we have implemented yet we are shure that its quite easy and fast to write beatiful slidenotes on the way. our goal is to find the right balance of control over the slidenote while keeping in mind that it has to be easy, intuitive and fast to write. with that slidenotes aims to be the fastest way to write your presentation.  

but we dont want to stop here and have some ideas how we could still improve - such as
- continuous headers to not write them again for every slide
- advanced hidden objects 

*did we miss something? any suggestions?
we are happy to hear your feedback*
||€€imagepart€€||
[{"names":["backgroundimage"],"filename":"imageupload.png","base64url":"data:image/jpeg;base64,"},{"names":["lapa"],"filename":"lapa.jpg","base64url":"data:image/jpeg;base64,"}]